...
const OrderModal = ({show, onHide, bundle, refill, country}: BundlesSectionProps) => {
    ...
    const [couponStatus, setCouponStatus] = React.useState<'valid' | 'invalid' | 'none'>('none');
    const router = useRouter();

    registerLocale('he', he);
    const {getValues, watch, control, handleSubmit, formState: {errors, isValidating}} = useForm({
        mode: 'onBlur',
    ...
    const handleCoupon = async () => {
        try {
            ...
            if (coupon) {
                setCouponSet(coupon);
                setLoadingCoupon(true);
                const couponResult = await fetch(`/api/coupon/${coupon}/${phoneNumber}`);
