...
type AdminTableProps<T extends GridValidRowModel> = {
    ...
    limit?: number;
    editMode?: 'row' | 'cell';
    addRow?: () => Promise<{ id: GridRowId, columnToFocus: string | undefined } | undefined>;
...
const AdminTable = <T extends GridValidRowModel>({
    ...
    const { apiRef, columns: columnsWithApi }: { apiRef: RefObject<GridApiCommunity>, columns: GridColumns } = useApiRef({ columns });
    const { columns: columnsWithActions }: { columns: GridColumns } = useAddActions({ columns: columnsWithApi, rowsActions: rowActions, rowModesModel, setRowModesModel, setRows: setDataRows, rows: dataRows });
