    ...
    return (
        <div className={styles.inputWrapper}>
            ...
            <div className={`d-flex position-absolute ${styles.inputEnd}`}>
                ...
                {error && (
                    <AnimatePresence>
                        <motion.div key={0} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className={styles.errorMessage}>
                            {error.message ? error.message as string : error as string}
                        </motion.div>
                        <motion.div key={1} initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }} className={styles.errorIcon}>
