...
const Login = ({ csrfToken }: { csrfToken: string | undefined }) => {
    ...
    const [ _, setCookie ] = useCookies(['phoneNumber'])
    const router = useRouter()

    const handleLogin = async () => {
        setLoading(true)
        setCookie('phoneNumber', phoneNumber)
        await signIn('email', { email: phoneNumber })
        setLoading(false)
    }
        ...
        <MainLayout hideJumbotron>
            <Form className={`${styles.signIn} d-flex flex-column align-items-center justify-content-center`}>
                ...
                <Form.Group className={styles.formGroup}>
                    ...
                    <Form.Control onChange={(e) => setPhoneNumber(e.target.value)} className={styles.buttonColor} type="text" name="email" id="email" size="lg" />
                </Form.Group>
                <motion.div layout>
