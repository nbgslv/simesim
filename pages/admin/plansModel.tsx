...
type PlansModelAsAdminTableData = (GridValidRowModel & Prisma.PlanModelMaxAggregateOutputType)[]
...
const PlansModel = ({ plansModel, existingBundles, existingRefills, existingCoupons }: PlansModelProps) => {
    const [plansRows, setPlansRows] = React.useState<PlansModelAsAdminTableData>(plansModel);
    const [vatToggleLoading, setVatToggleLoading] = React.useState<GridRowId>('');
    const [chosenBundle, setChosenBundle] = React.useState<Prisma.BundleMaxAggregateOutputType | null>(null);
    const [refills, setRefills] = React.useState<Prisma.RefillMaxAggregateOutputType[]>(existingRefills);
    ...
    const handleVatToggle = async (checked: boolean, rowId: GridRowId, row: GridValidRowModel) => {
        try {
            ...
            setPlansRows(plansRows.map((plan) => plan.id === rowId ? serializedUpdate : plan))
        } catch (e) {
            console.error(e)
        } finally {
            setVatToggleLoading('')
    ...
    return (
        <AdminLayout>
            ...
            />
            <FormModal id="add-plansmodel" {...bootstrapDialog(modal)} header={'Add Plan Model'}>
                <PlansModelForm
