...
const Coupons = ({ coupons, plansModel }: { coupons: CouponsAsAdminTableData, plansModel: PlanModel[] }) => {
    const [couponsRows, setCouponsRows] = React.useState<CouponsAsAdminTableData>(coupons);
    ...
    const columns: GridColumns = [
        ...
        },
        {
            field: 'discount',
            headerName: 'Discount',
            valueGetter: (params: any) => `${params.value}${params.row.discountType === 'PERCENT' ? '%' : '\u20AA'}`,
            editable: true
        },
        {
            field: 'discountType',
            ...
            renderEditCell: (params) => {
                ...
                return (
                    <DatePicker
                        ...
                        onChange={(date) => params.api.setEditCellValue({ id, field, value: format(date as Date, 'dd/MM/yyyy kk:mm') })}
                    />
                )
            }
        },
    ...
    return (
        <AdminLayout>
            <AdminTable
                ...
                addRow={showModal}
            />
            <FormModal id="add-coupons" {...bootstrapDialog(modal)} header={'Add Coupon'}>
                <CouponsForm
